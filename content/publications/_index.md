...
- **Method:** Built mine-road simulation scenarios and tyre, kinematic, and dynamic vehicle models; designed an MPC path-tracking controller on an improved kinematic model, with terminal penalty and constraint sets and Lyapunov-based stability analysis; designed dual-loop fuzzy-PID and ADRC acceleration tracking controllers with inverse longitudinal dynamics, engine and braking models, and a dead-zone throttle/brake switching mechanism; and integrated them into a dual-layer architecture combining upper-layer MPC path tracking with lower-layer ADRC acceleration control.
- **Result:** Full-condition simulations on typical mine paths show improved trajectory tracking accuracy, faster disturbance rejection, and stronger robustness under variable payload; the improved-kinematic-model MPC achieves a better balance between tracking accuracy and computational efficiency.

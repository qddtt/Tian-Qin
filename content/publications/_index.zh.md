...
- **方法：** 构建矿区仿真作业场景与车辆轮胎、运动学及动力学模型；基于改进型运动学模型设计带终端惩罚项与终端约束、并通过 Lyapunov 方法证明稳定性的 MPC 路径跟踪控制器；设计双环模糊 PID 与 ADRC 加速度跟踪控制器，建立逆纵向动力学、发动机与制动模型并引入油门/刹车切换死区机制；最终集成为“上层 MPC 路径跟踪 + 下层 ADRC 加速度控制”的双层控制架构。
- **结果：** 典型矿区路径全工况仿真表明，所提方案在轨迹跟踪精度、动态响应速度与抗扰鲁棒性上表现更优；改进运动学模型 MPC 在控制精度与运算效率间取得较优平衡。
